...
import 'package:eshopmultivendor/Screen/AddTimeSlots.dart';
import 'package:eshopmultivendor/Screen/Add_Product.dart';
import 'package:eshopmultivendor/Screen/Authentication/Login.dart';
import 'package:eshopmultivendor/Screen/CustomerSupport.dart';
import 'package:eshopmultivendor/Screen/MyQrScreen.dart';
import 'package:eshopmultivendor/Screen/StaffList.dart';
    ...
    return LineChartData(
      ...
      titlesData: FlTitlesData(
        ...
        bottomTitles: SideTitles(
          ...
          margin: 10,
          getTitles: (value) {
            return months![value.toInt()].toString();
          },
        ),
      ),
      gridData: FlGridData(
    ...
    apiBaseHelper.postAPICall(getStatisticsApi, parameter).then(
      (getdata) async {
        ...
        if (!error) {
          ...
          print("checking_month_____${getdata['earnings'][0]["monthly_earnings"]['month_name']}");

          if(getdata['category_wise_product_count'] != null) {
    ...
    if (_isNetworkAvail) {
      ...
      apiBaseHelper.postAPICall(getSellerDetails, parameter).then(
        (getdata) async {
          ...
          getdata["delivery"].toString() == "1" ? delivery_onOf = true : delivery_onOf = false;


          if (!error) {
            var data = getdata["data"][0];
            print('getSellerDetails_response______________ $data');
  ...
  Future<void> shopStatus() async {
    ...
    if (_isNetworkAvail) {
      ...
      apiBaseHelper
          ...
          .then((getdata) async {
        bool error = getdata["error"];
        print('-----Surendra-------${updateUserApi}------${parameter}--------');
        if (!error) {
          setState(() {
            print("Success");
          });
  ...
  getAppBar(BuildContext context) {
    return PreferredSize(
      ...
      child: AppBar(
        centerTitle: true,
        title:
        isLoading == true && (appLogo != null || appLogo != "")? Container():
        ...
        child: Container(
          ...
          child: ListView(
            ...
            children: <Widget>[
              ...
              // _getDrawerItem(19, getTranslated(context, "CUSTOMER_SUPPORT")!, Icons.person),
              _getDrawerItem(
                  16, getTranslated(context, "My Qr")!, Icons.qr_code),
              ...
              _getDrawerItem(20, getTranslated(context, "DELIVERY")!, Icons.production_quantity_limits_outlined),
              _getDrawerItem(10, getTranslated(context, 'Add Product')!, Icons.add),
              _getDrawerItem(11, getTranslated(context, 'AddTimeSlots')!, Icons.timelapse),
              Divider(),
...
//  => Drawer Item List
  ...
  _getDrawerItem(int index, String title, IconData icn) {
    return Container(
      ...
      child: ListTile(
        ...
        trailing: title == getTranslated(context, "DELIVERY")? Padding(
          padding: const EdgeInsets.only(left: 8.0, right: 10),
          child: CupertinoSwitch(
              trackColor: primary,
              value: delivery_onOf,
              onChanged: (value) {
                setState(() {
                  delivery_onOf = value;
                  print("delivery_onOf___${delivery_onOf}");
                });
                deliverOnApi();
              }),
        )
        ...
        onTap: () {
          if (title == getTranslated(context, "HOME")!) {
          ...
          else if (title == getTranslated(context, "My Qr")!) {
            setState(
                  () {
                curDrwSel = index;
              },
          ...
          else if (title == getTranslated(context, "Staff")!) {
            setState(
              () {
                curDrwSel = index;
              },
          ...
          } else if (title == getTranslated(context, "Create_staff")!) {
            setState(
              () {
                curDrwSel = index;
              },
            ...
            Navigator.pop(context);
            Navigator.push(context, MaterialPageRoute(builder: (context) => CreateStaff(isEdit: false),),
            );
          }
          // else if (title == getTranslated(context, "Manage_staff")!) {
          //   setState(
          //     () {
          //       curDrwSel = index;
          //     },
          //   );
          //   Navigator.pop(context);
          //   Navigator.push(context, MaterialPageRoute(builder: (context) => CreateStaff(isEdit: true),),
          //   );
          // }
            ...
            setState(() {curDrwSel = index;},);
            Navigator.pop(context);
            Navigator.push(context, MaterialPageRoute(builder: (context) => AddProduct(),),
            );
          } else if (title == getTranslated(context, "AddTimeSlots")!) {
            setState(() {curDrwSel = index;},);
            Navigator.pop(context);
            Navigator.push(context, MaterialPageRoute(builder: (context) => AddTimeSlots(),),
            );
          }
        },
      ),
  ...
  deliverOnApi() async{
    Map<String, dynamic> params = {
      ...
      'status': delivery_onOf == true? "1" : "0",
    };

    print("update_deliveryApi_is__ ${update_deliveryApi} & params___ $params");

    final response = await http.post(update_deliveryApi, body: params);
    var jsonResponse = convert.jsonDecode(response.body);

    if(jsonResponse["error"] == false){
      jsonResponse["data"]['order_delivery'].toString() == "1" ? delivery_onOf = true : delivery_onOf = false;
          print("update_deliveryApi_response_is__ ${jsonResponse["data"]} delivery_onOf______________ $delivery_onOf");
    }
    else{
